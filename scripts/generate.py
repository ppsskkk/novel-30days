...
for idx, seg in enumerate(segments, 1):
    r = client.chat.completions.create(
        ...
        messages=[{"role": "user", "content":
            ...
            f"2）原著原文摘录9~11段：优先选关键对话、心理描写、戳人的细节描写、"
            f"有张力的冲突场景，每段100~250字，原文照录，注明出自什么情境\n\n"
            f"片段如下：\n{seg}"}],
    )
...
4. 【核心要求】这是一篇"带原文的解读"，全文的原著原文占比不低于二分之一：
   - 今日情节：按故事发展顺序，每个关键情节点都先引一段原文（引用块），再用一两句话点评，至少引6~8段
   - 人物与细节分析、主题深挖：每个板块至少嵌入3~5段原文，引用后紧跟你的点评
   - 金句摘抄：集中放4~5段最精彩的原文
